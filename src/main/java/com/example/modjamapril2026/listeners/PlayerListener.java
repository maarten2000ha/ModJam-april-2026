package com.example.modjamapril2026.listeners;
...
public class PlayerListener {
    ...
    public void register(EventRegistry eventBus) {
        ...
        try {
            ...
            LOGGER.at(Level.INFO).log("[mod jam april 2026] Registered PlayerConnectEvent listener");
        } catch (Exception e) {
            LOGGER.at(Level.WARNING).withCause(e).log("[mod jam april 2026] Failed to register PlayerConnectEvent");
        }

        // PlayerDisconnectEvent - When a player disconnects
        try {
            eventBus.register(PlayerDisconnectEvent.class, this::onPlayerDisconnect);
            LOGGER.at(Level.INFO).log("[mod jam april 2026] Registered PlayerDisconnectEvent listener");
        } catch (Exception e) {
            LOGGER.at(Level.WARNING).withCause(e).log("[mod jam april 2026] Failed to register PlayerDisconnectEvent");
        }
    }
    ...
    private void onPlayerConnect(PlayerConnectEvent event) {
        ...
        LOGGER.at(Level.INFO).log("[mod jam april 2026] Player %s connected to world %s", playerName, worldName);
    ...
    private void onPlayerDisconnect(PlayerDisconnectEvent event) {
        ...
        LOGGER.at(Level.INFO).log("[mod jam april 2026] Player %s disconnected", playerName);
